  ...
  return (
    <Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl max-h-[80vh] md:max-h-[75vh] flex flex-col p-0 gap-0">
        {/* Header */}
        <div className="flex items-center justify-between px-4 md:px-6 py-3.5 md:py-4 border-b border-border shrink-0">
          <div className="flex items-center gap-2.5 md:gap-3">
            <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-accent/10 text-accent flex items-center justify-center">
              <NavigationArrow className="w-4 h-4 md:w-5 md:h-5" />
            </div>
            <div>
              <h3 className="max-md:text-[17px] md:text-base font-bold text-foreground">
                วางแผนเส้นทาง
              </h3>
              {routeData && (
                <p className="max-md:text-[14px] md:text-[15px] text-muted-foreground">
        ...
        {showManualOrigin && !routeData && (
          <div className="p-6 md:p-8 text-center space-y-4">
            <MapPin className="w-9 h-9 md:w-10 md:h-10 mx-auto mb-1 text-muted-foreground opacity-60" />
            <p className="max-md:text-[16px] md:text-[17px] font-medium text-foreground">
            ...
            <div className="flex gap-2 max-w-xs mx-auto">
              <input
                ...
                className="flex-1 h-9 px-3 text-[16px] font-sans rounded-[4px] bg-card text-foreground outline-none focus:border-ring"
              />
              <input
                type="number"
                step="any"
            ...
            {routeError && (
              <p className="text-[14px] text-destructive">{routeError}</p>
            )}
        ...
        {!showManualOrigin && routeError && !routeData && (
          <div className="p-8 md:p-10 text-center">
            <MapPin className="w-9 h-9 md:w-10 md:h-10 mx-auto mb-3 text-destructive opacity-60" />
            <p className="max-md:text-[17px] md:text-base font-medium text-destructive">
              {routeError}
            </p>
        ...
        {routeData && (
          <>
            <div className="flex-1 overflow-y-auto px-4 md:px-5 py-3 md:py-4 space-y-2 md:space-y-3">
              {/* Origin */}
              <div className="flex items-center gap-3 md:gap-4 px-3 md:px-4 py-2.5 md:py-3 rounded-[6px] md:rounded-[8px] bg-card">
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-primary text-white flex items-center justify-center max-md:text-[14px] md:text-[15px] font-bold shrink-0">
                  A
                </div>
                <div className="flex-1 min-w-0">
                  <div className="max-md:text-[15px] md:text-[16px] font-medium text-foreground">
                    ตำแหน่งปัจจุบัน
                  </div>
                  <div className="font-mono max-md:text-[13px] md:text-[14px] text-muted-foreground">
              ...
              {routeData.clients.map((item, i) => (
                <div
                  key={item.client.id}
                  className="flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2.5 md:py-3 rounded-[6px] md:rounded-[8px] hover:bg-card transition-colors"
                >
            ...
            {/* Footer */}
            <div className="px-4 md:px-5 py-3 md:py-4 border-t border-border shrink-0">
              <a
                href={googleMapsUrl(
